...
def creatpdf(name, event):
   ...
   # To find the x and y axes.
    for x in range(1000):
       can.drawString(x*20, 85, str(x*20)) # x axis
       can.drawString(30, x*20, str(x*20)) # y axis
    can.save()
    ...
    existing_pdf = PdfFileReader(open("ticket with name.pdf", "rb"))
    ...
    outputStream = open("cords.pdf", "wb")
